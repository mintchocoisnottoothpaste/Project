...
KORdata <- hmd.mx(country = "KOR", username = "rjmorpheus@gmail.com",password = "Password", label = "Republic of Korea")

KORStMoMO_m <- StMoMoData(KORdata,series = "male")
KORStMoMO_f <- StMoMoData(KORdata,series = "female")
...
m_mat <- matrix(m, nrow = n.x, ncol = n.y) # 111 X 16
plot(m_mat[,1], type = "l") #mortality of all ages in year 2003
...
# as exxpected
plot(x = Years, y = m_mat[65,], pch = 18, xlab = "Years", ylab = "log m", col = 2)
...
m_hat <- matrix(1, nrow = n.y, ncol = n.x) #Dummy matrix with dim 16 X 111
m_hat <- t(m_mat)
m_mat <- t(m_mat)
...
for(j in 1:100) S[,j] <- -15.919488 + cumsum(-0.75 + rnorm(23, 0, 0.75))
...
ggplot() +
  ...
  geom_line(data = NULL, aes(x = 2018:2040, y = sim[1,]), col = 2) +
  geom_line(data = NULL, aes(x = 2018:2040, y = sim[2,]), col = 4) +
  geom_line(data = NULL, aes(x = 2018:2040, y = sim[3,]), col = 2)
...
LC1 <- lc(link = "logit")
data_m <- central2initial(KORStMoMO_m)
data_f <- central2initial(KORStMoMO_f)
ages_fit <- 40:90
...
#Forecasting----
LCfore_m <- forecast(LCfit_m, h = 23)
LCfore_f <- forecast(LCfit_f, h = 23) 
...
par(mfrow=c(1,3))
plot(x = Years, y = LCfit_m$kt[1,], type = "l", xlim = c(2003,2045),
     ylim = c(-70,20), sub = "a)", ylab = "kt")
lines(x = 2018:2040, y = LCfore_m$kt.f$mean, col = 2)
lines(x = 2018:2040, y = LCfore_m$kt.f$upper[1,,1], col = 4)
lines(x = 2018:2040, y = LCfore_m$kt.f$lower[1,,1], col = 4)

plot(x = Years, y = k_1, xlab = "Years",type = "l", ylab = "kt",
     xlim = c(2003,2045), ylim = c(-90,50), sub = "b)")
lines(x = 2018:2040, y = sim[1,], col = 4, lty = "dashed")
lines(x = 2018:2040, y = sim[2,], col = 2, lty = "dotted")
lines(x = 2018:2040, y = sim[3,], col = 4, lty = "dashed")

LCforeArima <- forecast(LCfit_m, h = 23, kt.method = "iarima",
                        kt.order = c(1, 1, 2))
...
## Simulation for LC
LCsim_m <- simulate(LCfit_m, nsim = 500, h = 23)
LCsim_f <- simulate(LCfit_f, nsim = 500, h = 23)
...
plot(LCfit_m$years, exp(m[75, ]), xlim = range(LCfit_m$years, LCfit_m$years),
...
plot(LCfit_f$years, f[75, ], xlim = range(LCfit_f$years, LCfit_f$years),
     ylim = b_ ,type = "l",
